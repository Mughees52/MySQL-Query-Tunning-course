...
-- STEP 1 · [SLIDE 7, card 1] subquery in SELECT — then EXPLAIN: "run only once"
SELECT ship_country,
...
-- STEP 3 · [SLIDE 7, card 2] subquery in WHERE — a threshold computed at run time
-- expect: 543009
SELECT COUNT(*) FROM orders
WHERE total_cents > (SELECT AVG(total_cents) FROM orders);

-- STEP 4 · [SLIDE 8] the CTE — read it aloud top-to-bottom, then run (~0.5 s)
WITH country_rev AS (
